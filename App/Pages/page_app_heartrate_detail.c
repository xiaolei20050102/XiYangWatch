...
#define HR_N   24
#define CURVE_N 24
static const int hr_24h[HR_N] = {
    52, 50, 48, 49, 51, 55, 62, 75,
    88,105,120, 98, 85,110,145,185,
   200,170,130, 95, 78, 68, 60, 55
};
...
static lv_obj_t *create(lv_obj_t *parent)
{
    ...
    lv_label_set_text(title, "Heart Rate | 24h");
    ...
    calc_stats(hr_24h, HR_N, &h_min, &h_max, &h_avg);
    ...
    lv_obj_set_style_text_color(stat_avg, lv_color_hex(0xFF3B4A), 0);
    ...
    gen_curve(hr_24h, HR_N, hr_pts, CURVE_N,
              cx, chart_y, cw, chart_h, 30, 210);

    lv_obj_t *line = lv_line_create(card);
    lv_line_set_points_mutable(line, hr_pts, CURVE_N);
    lv_obj_set_style_line_color(line, lv_color_hex(0xFF3B4A), 0);
    lv_obj_set_style_line_width(line, 2, 0);

    /* 终点圆点 */
    lv_point_precise_t last = hr_pts[CURVE_N - 1];
    lv_obj_t *dot = lv_obj_create(card);
    lv_obj_set_size(dot, 6, 6);
    lv_obj_set_pos(dot, (lv_coord_t)last.x - 3, (lv_coord_t)last.y - 3);
    lv_obj_set_style_radius(dot, 3, 0);
    lv_obj_set_style_bg_color(dot, lv_color_hex(0xFF3B4A), 0);
...
const page_t page_app_heartrate_detail = {
    .name    = "heartrate_detail",
